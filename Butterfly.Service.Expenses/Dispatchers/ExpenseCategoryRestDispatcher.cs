...
{
    public class ExpenseCategoryRestDispatcher : RestDispatcher
    {
        ...
        {
            List<ExpenseCategory> categories = ExpensesDataService.GetExpenseCategories();
            Serializer.Serialize(categories, context.Response.OutputStream);
        }

        public override void POST(HttpContext context)
        {
            CreateExpenseCategoryRequest request = Serializer.Deserialize(context.Request.InputStream, typeof(CreateExpenseCategoryRequest)) as CreateExpenseCategoryRequest;
            BaseResult result = new BaseResult();
            if (request != null)
            {
                Response response = ExpensesDataService.CreateExpenseCategory(request);
                if (response != null)
                {
        ...
        {
            DeleteExpenseCategoryRequest request = Serializer.Deserialize(context.Request.InputStream, typeof(DeleteExpenseCategoryRequest)) as DeleteExpenseCategoryRequest;
            BaseResult result = new BaseResult();
            if (request != null)
            {
                Response response = ExpensesDataService.DeleteExpenseCategory(request);
                if (response != null)
                {
                    switch (response.ResultCode)
                    {
